---
helpx_url: "https://helpx.adobe.com/es/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/texture-generators/noises/fur-1.html"
breadcrumb-title: ""
description: Usa el nodo Piel 1 para generar texturas básicas de piel para crear efectos de cabello, tejido y materiales fibrosos.
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Texture Generators > Noises > Fur 1
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Piel 1
user-guide-description: ""
user-guide-title: ""
source-git-commit: 0f214099ae94088d37122a5d474d3e70d4ccf46f
workflow-type: tm+mt
source-wordcount: '77'
ht-degree: 9%
---

# Piel 1

<table>
<tr style="border: 0;">
<td width="33.33%" style="border: 0;" valign="top">

![](fur-1.resources/fur-1.png){width="128px"}

<b>En:</b> Generadores de Textura > Ruidos

</td>
<td width="100.00%" style="border: 0;" valign="top">

## Descripción

Esto genera un tipo de ruido de piel recta.

</td>
</tr>
</table>

<a name="parameters"></a>

## Parámetros

|  |  |
|:---|:---|
| <b>Escala</b> <i>1 - 8</i> | Establece la escala global del efecto. |
| <b>Desorden</b> <i>0.0 - 1.0</i> | Desplaza la fase del ruido para introducir pequeñas variaciones. |
| <b>Expansión no cuadrada</b> <i>Falso/Verdadero</i> | Permite la compensación de aplastamiento y estiramiento con proporciones no cuadradas. |

## Ejemplos

<table style="table-layout:fixed">
    <tr style="border: 0;">
        <td style="border: 0;">
            <img src="fur-1.resources/fur-1-ex.gif" class="modal-image" alt="Piel 1 - Ejemplo 1" />
        </td>
        <td style="border: 0;"></td>
        <td style="border: 0;"></td>
    </tr>
</table>
